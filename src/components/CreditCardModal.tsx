...
export const CreditCardModal = memo<CreditCardProps>(({ isOpen, onClose, onConfirm, cardNumber, setCardNumber, expiry, setExpiry, cvc, setCvc }) => {
    ...
    const handleCardNumberChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        setCardNumber(event.target.value)
    }, [setCardNumber]);

    const handleExpiryChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        setExpiry(event.target.value)
    }, [setExpiry]);    

    const handleCvcChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
