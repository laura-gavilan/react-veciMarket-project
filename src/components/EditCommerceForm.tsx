...
export type EditCommerceFormProps = {
    form: CommerceForm;
    // setForm3: (newValue: unknown) => void;
    // setForm2: React.Dispatch<React.SetStateAction<CommerceForm>>
    setForm: Dispatch<SetStateAction<CommerceForm>>
    onSubmit: (form: CommerceForm) => void;
};

export const EditCommerceForm = forwardRef(({ form, setForm, onSubmit }: EditCommerceFormProps, ref) => {
    const firstInputRef = useRef<HTMLInputElement | null>(null);
    const { t } = useTranslate();

    useImperativeHandle(ref, () => ({
        reset: () => {
            setForm(INITIAL_COMMERCE_FORM);
        },
    ...
    // const handleChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    //     const { name, value } = event.target;
    //     setForm((prev) => ({ ...(prev as CommerceForm), address: { ...(prev as CommerceForm).address, [name]: value } }));
    // };
