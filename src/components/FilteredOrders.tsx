import { memo, useMemo } from "react";
import { useTranslate } from "../translations/locales/useTranslate";
import type { Orders, User } from "../types/types";

export type OrderStatus = | "pending" | "preparing" | "delivered" | "cancelled";

export type StatusOption = {
    value: OrderStatus | "all";
    label: string;
    color: string;
};

export type FilteredOrdersProps = {
    filteredOrders: Orders[];
    statusOptions: StatusOption[],
    statusLabels: Record<OrderStatus, string>;
    notesState: Record<string, string>;
    handleNotesChange: (orderId: string, value: string) => void;
    saveNotes: (orderId: string) => void;
    getStatusColor: (status: OrderStatus) => string;
    getTotals: (order: Orders) => { subtotal: number, tax: number, total: number };
    updateOrderStatus: (orderId: string, status: OrderStatus) => void;
    deleteOrder: (orderId: string) => void,
    canEdit: boolean,
    user: User,
};

// export type FilteredOrdersProps<TOrder extends { _id: string; userId: string; status: OrderStatus; items: any[]; }, TUser extends { _id: string; role?: string }> = {
//     filteredOrders: TOrder[];
//     statusOptions: StatusOption[];
//     statusLabels: Record<OrderStatus, string>;
//     notesState: Record<string, string>;
//     handleNotesChange: (orderId: TOrder["_id"], value: string) => void;
//     saveNotes: (orderId: TOrder["_id"]) => void;
//     getStatusColor: (status: OrderStatus) => string;
//     getTotals: (order: TOrder) => { subtotal: number; tax: number; total: number };
//     updateOrderStatus: (orderId: TOrder["_id"], status: OrderStatus) => void;
//     deleteOrder: (orderId: TOrder["_id"]) => void;
//     canEdit: boolean;
//     user: TUser;
// };


// export const FilteredOrders = memo(<TOrder extends { _id: string; userId: string; status: OrderStatus; items: any[] }, TUser extends { _id: string; role?: string }>({}: FilteredOrdersProps<TOrder, TUser>) => {....}
export const FilteredOrders = memo<FilteredOrdersProps>(({ filteredOrders, statusOptions, statusLabels, notesState, handleNotesChange, saveNotes, getStatusColor, getTotals, updateOrderStatus, deleteOrder, canEdit = false, user }) => {
    const isAdmin = user?.role === "admin";
    const { t } = useTranslate();

    const visibleOrders = useMemo(() => {
        return filteredOrders.filter(order => {
            if (isAdmin) return true;
            if (user?._id) return order.userId === user._id;
            return false;
        });
    }, [filteredOrders, user, isAdmin]);

    // const handleNote = (orderId: TOrder["_id"], value: string) => handleNotesChange(orderId, value);
    const handleNote = (orderId: string, value: string) =>
        handleNotesChange(orderId, value)

    const orders = useMemo(() => {
        return visibleOrders.map(order => {
            const { subtotal, tax, total } = getTotals(order);
            const noteValue = notesState[order._id] ?? order.notes ?? "";
            const statusColor = getStatusColor(order.status).split(' ')[0];
            const hasProducts = Array.isArray(order.items) && order.items.length > 0;

            return (
                <div
                    key={order._id}
                    className="border rounded-md p-4 flex flex-col justify-between font-sans min-h-[220px] shadow-sm hover:shadow-md transition bg-general md:max-w-md"
                >
                    <div className={`w-4 h-4 rounded-full ${statusColor} self-end mb-2`}></div>

                    <div className="flex justify-start text-xs text-primary-light mb-2">
                        {order.createdAt && new Date(order.createdAt).toLocaleString()}
                    </div>

                    <div className="text-[11px] mb-3 space-y-1 text-primary-dark">
                        <p><strong>ID:</strong> {order._id}</p>
                        <p><strong>{t("user.user")}:</strong> {order.userId}</p>
                        <p><strong>{t("components.state")}:</strong>
                            <span className={`ml-1 px-2 py-1 rounded text-[10px] font-semibold ${getStatusColor(order.status)} ${statusOptions.find(s => s.value === order.status)?.color.split(' ')[1]}`}>
                                {statusLabels[order.status] || order.status}
                            </span>
                        </p>
                    </div>

                    <div className="mb-2">
                        {canEdit && isAdmin && (
                            <>
                                <textarea
                                    value={noteValue}
                                    onChange={(event) => handleNote(order._id, event.target.value)}
                                    className="w-full border border-gray-300 rounded px-2 py-1 resize-none text-sm focus:outline-none focus:ring-1 focus:ring-[var(--color-mostaza)]"
                                    placeholder="Notas..."
                                    rows={2}
                                />
                                <div className="flex justify-center mt-2">
                                    <button
                                        onClick={() => saveNotes(order._id)}
                                        className="px-3 py-1 text-xs rounded-full btn-primary"
                                    >
                                        {t("components.save")}
                                    </button>
                                </div>
                            </>
                        )}
                        {!canEdit || !isAdmin && (
                            <textarea
                                value={noteValue}
                                readOnly
                                className="w-full border border-gray-300 rounded px-2 py-1 resize-none text-sm bg-gray-100"
                                rows={2}
                            />
                        )}
                    </div>

                    <div className="border border-gray-300 p-2 rounded mb-2 flex-1 overflow-auto text-sm space-y-1">
                        {hasProducts && order.items.map((item, index) => {
                            const isLast = index === order.items.length - 1;
                            return (
                                <div key={index}
                                    className={`flex justify-between pb-1 mb-1 ${!isLast ? "border-b border-gray-300" : ""}`}
                                >
                                    <span><strong>{item.name ?? `Producto ${item.productId}`}</strong>
                                        ({item.qty ?? 0}×€{(item.price ?? 0).toFixed(2)})
                                    </span>
                                    <span>€{((item.price ?? 0) * (item.qty ?? 0)).toFixed(2)}</span>
                                </div>
                            );
                        })}
                        {!hasProducts && <p className="italic text-primary-light">Sin productos</p>}
                    </div>

                    <div className="bg-yellow-100 p-2 rounded mb-2 text-sm font-sans space-y-1">
                        <p><strong>{t("cart.subtotal")}:</strong> €{subtotal.toFixed(2)}</p>
                        <p><strong>{t("orders.taxes")}: (10%):</strong> €{tax.toFixed(2)}</p>
                        <p><strong>{t("cart.total")}:</strong> €{total.toFixed(2)}</p>
                    </div>

                    {canEdit && isAdmin && (
                        <div className="space-y-1">
                            <select
                                value={order.status}
                                onChange={(event) => updateOrderStatus(order._id, event.target.value as OrderStatus)}
                                className="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-[var(--color-mostaza)]"
                            >
                                {statusOptions.filter(statusOption => statusOption.value !== "all").map(status => {
                                    return <option key={status.value} value={status.value}>{status.label}</option>
                                })}
                            </select>
                            <div className="flex justify-center mt-1">
                                <button
                                    onClick={() => deleteOrder(order._id)}
                                    className="bg-red-500 text-white rounded px-2 py-1 text-xs hover:bg-red-600 transition-colors"
                                >
                                    X
                                </button>
                            </div>
                        </div>
                    )}

                </div>
            )
        })
    }, [visibleOrders, notesState, statusLabels, statusOptions, canEdit, isAdmin, handleNotesChange, saveNotes, getStatusColor, getTotals, updateOrderStatus, deleteOrder]);

    return (
        <div className="flex flex-col gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-2">
                {orders}
            </div>
        </div>
    )
});