...
export const INITIAL_FORM_REGISTER: RegisterForm = {
    username: "",
    firstName: "",
    lastName: "",
    name: "", 
...
const Register = () => {
    ...
    const [form, setForm] = useState<RegisterForm>(INITIAL_FORM_REGISTER);
    const [error, setError] = useState<string | null>(null);
    const usernameRef = useRef<HTMLInputElement>(null);
    const { t } = useTranslate();

    const REGISTER_FORM: {
        input: { name: keyof RegisterForm; type: string; placeholder: string; required: boolean };
        label: { text: string };
    }[] = [
            { input: { name: "username" as keyof RegisterForm, type: "text", placeholder: t("pages.register.form.username.placeholder"), required: true }, label: { text: t("pages.register.form.username.label") } },
    ...
    const onRegisterSubmit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
